...
export class ChatsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  broadcastMessage(userId: string, message: any) {
    ...
    if (socket) {
      console.log(
        `   ✅ Socket found for user ${userId}, emitting 'newMessage'`,
      );
      socket.emit('newMessage', message);
      return true; // Online
    }
    console.log(
      `   ❌ Socket NOT found for user ${userId} - user appears offline`,
    );
  ...
  async handleSendMessage(
    ...
    console.log(
      `📨 [WebSocket] sendMessage from ${sender.fullName} (${sender.id})`,
    );
